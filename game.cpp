...
{	
	if ((OTSYS_TIME() - lastUseTime) < 100) {
		return;
	}

	// Em algumas partes do código tem return, e ele pode acabar não continuando pro resto do código, mais garantido aqui.
	lastUseTime = OTSYS_TIME();
	...
	if (type == MARKETACTION_SELL) {
		...
		if (it.stackable) {
			if (it.id != ITEM_TIBIACOIN) {
				...
				uint32_t accId = player->getAccount();
				DBResult_ptr result = db->storeQuery("SELECT `premium_points` FROM `accounts` WHERE `id`=" + std::to_string(accId));
				if (!result) {
					return;
				}
				...
				if (result->getNumber<uint32_t>("premium_points") < amount) {
					return;
				}

				db->executeQuery("UPDATE `accounts` SET `premium_points`=`premium_points`-" + std::to_string(amount) + " WHERE `id`=" + std::to_string(accId));
			}
		} else {
			for (Item* item : itemList) {
				internalRemoveItem(item);
	...
	if (offer.type == MARKETACTION_BUY) {
		...
		if (it.stackable) {
			if (it.id != ITEM_TIBIACOIN) {
				...
				Database* db = Database::getInstance();
				db->executeQuery("UPDATE `accounts` SET `premium_points`=`premium_points`+" + std::to_string(offer.amount) + " WHERE `id`=" + std::to_string(player->getAccount()));
			}
		} else {
			int32_t subType;
			if (it.charges != 0) {
	...
	if (offer.type == MARKETACTION_BUY) {
		...
		if (it.stackable) {
			if (it.id != ITEM_TIBIACOIN) {
				...
				Database* db = Database::getInstance();
				db->executeQuery("UPDATE `accounts` SET `premium_points`=`premium_points`-" + std::to_string(amount) + " WHERE `id`=" + std::to_string(player->getAccount()));
			}
		} else {
			for (Item* item : itemList) {
				internalRemoveItem(item);
		...
		if (it.stackable) {
			if (it.id != ITEM_TIBIACOIN) {
				...
				isTibiaCoin = true;
				Database* db = Database::getInstance();
				db->executeQuery("UPDATE `accounts` SET `premium_points`=`premium_points`+" + std::to_string(amount) + " WHERE `id`=" + std::to_string(buyerPlayer->getAccount()));
			}
		} else {
			int32_t subType;
			if (it.charges != 0) {
